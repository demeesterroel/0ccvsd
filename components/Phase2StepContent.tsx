...
export default function Phase2StepContent({ stepId }: { stepId: string }) {
    const router = useRouter();
    const { markVisited } = useNavigation();

    useEffect(() => {
        if (stepId) {
            markVisited(`p2-s${stepId}` as any);
        }
    }, [stepId, markVisited]);
            ...
            <footer className="border-t-2 border-ink pt-12 flex flex-col md:flex-row gap-8">
                <button
                    onClick={() => {
                        ...
                        router.push(prev > 0 ? `/phase2/step/${prev}` : '/phase2');
                    }}
                ...
                >
                    ...
                    <div>
                        ...
                        <h4 className="text-xl font-bold">{stepNum === 1 ? 'Phase II: Development' : `Step 0${stepNum - 1}`}</h4>
                    </div>
                </button>
                <button
                    onClick={() => {
                ...
                >
                    <div className="flex items-center gap-6">
                        <div className="size-14 border border-white/30 flex items-center justify-center">
                            <span className="material-symbols-outlined text-3xl">{stepNum === 5 ? 'rule' : 'architecture'}</span>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-60">{stepNum === 5 ? 'Phase III Transition' : 'Next Step'}</p>
                            <h4 className="text-2xl font-bold italic">{stepNum === 5 ? 'Integration' : `Step 0${stepNum + 1}`}</h4>
                        </div>
                    </div>
