...
const PhaseOverview: React.FC<PhaseOverviewProps> = ({ phase }) => {
    const router = useRouter();
    const { markVisited } = useNavigation();

    useEffect(() => {
        markVisited(`phase${phase}` as any); // Type cast if necessary based on your PageId type
    }, [phase, markVisited]);
    ...
    return (
        <div className="max-w-6xl mx-auto px-6 md:px-16 py-10 md:py-20">
            ...
            <section className="mb-16">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {content.steps.map((step, i) => (
                        <button key={i} onClick={() => router.push(step.path)} className="hand-drawn-border p-8 bg-white hover:bg-cream transition-colors group text-left">
            ...
            <footer className="border-t border-ink pt-16 flex flex-col md:flex-row gap-8">
                <button onClick={() => router.push(content.prev)} className="flex-1 p-8 border-2 border-ink hover:bg-ink hover:text-white transition-all group text-left">
                ...
                <button onClick={() => router.push(content.next)} className="flex-1 p-8 border-2 border-ink hover:bg-ink hover:text-white transition-all group text-left">
