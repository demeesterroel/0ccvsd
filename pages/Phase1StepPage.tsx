...
const Phase1StepPage: React.FC<{ stepId: string, onNavigate: (id: PageId) => void }> = ({ stepId, onNavigate }) => {
  ...
  const step = stepData[stepId as keyof typeof stepData] || stepData['p1-s1'];
      ...
      <footer className="border-t-2 border-ink pt-12 flex flex-col md:flex-row gap-8">
        ...
        >
          ...
          <div>
            ...
            <h4 className="text-xl font-bold">{parseInt(stepNum) === 1 ? 'Phase I: Exploration' : `Step 0${parseInt(stepNum) - 1}`}</h4>
          </div>
        </button>
        <button
          onClick={() => {
        ...
        >
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold italic">{parseInt(stepNum) === 6 ? 'Development' : `Step 0${parseInt(stepNum) + 1}`}</h4>
            </div>
          </div>
